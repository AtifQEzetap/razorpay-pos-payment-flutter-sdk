...
class EzeSdkFlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        ...
        when (call.method) {
            ...
            "getCardInfo" -> getCardInfo(call.arguments.toString())
    ...
    private fun getCardInfo(jsonObject: String?) {
        try {
            EzeAPI.getCardInfo(activity, GET_CARD_INFO, jsonObject?.let { JSONObject(it) })
        } catch (e: JSONException) {
            e.printStackTrace()
            ezeResult.error("EZE_ERROR", e.message, "")
        }
    ...
    private fun searchTransaction(requestObject: String?) {
        try {
            EzeAPI.searchTransaction(activity,
    ...
    private fun getTransaction(externalReference: String?) {
        try {
            EzeAPI.getTransaction(activity,
    ...
    private fun attachSignature(requestObject: String?) {
        try {
            EzeAPI.attachSignature(activity,
